...
# --- 4. Model Selection, Tuning, and Cross-Validation (LITE VERSION) ---
print("\n--- 4. Starting Model Training, Tuning, and Evaluation (LITE VERSION) ---")
# Define models and their hyperparameter grids for GridSearchCV
# These grids have been reduced to cut down on training time.
models_and_params = {
    'RandomForestRegressor': {
        'model': RandomForestRegressor(random_state=42, n_jobs=-1),
        'params': {
            'n_estimators': [50, 100],  # Reduced options
            'max_depth': [10, 20],      # Reduced options, removed 'None' for speed
    ...
    'XGBRegressor': {
        ...
        'params': {
            'n_estimators': [50, 100],      # Reduced options
            'learning_rate': [0.1],         # Fixed to one common value
            'max_depth': [5],               # Fixed to one common value
            'subsample': [0.8]              # Fixed to one common value
        }
    },
    'GradientBoostingRegressor': {
        ...
        'params': {
            'n_estimators': [50, 100],      # Reduced options
            'learning_rate': [0.1],         # Fixed to one common value
            'max_depth': [3]                # Fixed to one common value
        }
    }
}
